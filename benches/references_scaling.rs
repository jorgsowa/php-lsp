...
//!
//! Models the real cost driver behind a 5-41s references cliff observed on
//! generic short names (`Color`, `Node`, `Asset`, ...): a *common* public
//! method name (`process`) shared across unrelated classes, where only a
//! fraction of the text-matching files actually reference the target
//! `App\Service`.
//!
...
fn main() {
    ...
    for &n in &[100usize, 500, 1000, 3000] {
        let (before_n, before_ms) =
            cold_ms(n, reps, &sym, |s, _| s.workspace_file_paths().to_vec());
