...
- **PHPDoc type system** — full docblock support: `@param`, `@return`, `@var`, `@throws`, `@deprecated`, `@see`, `@link`; `@template T` / `@template T of Base` generics; `@mixin ClassName`; `@psalm-type` / `@phpstan-type` type aliases; `@property`/`@property-read`/`@property-write` and `@method [static]` tags — synthesised members appear in `->` completions; callable type signatures `callable(int, string): void` parsed correctly; **Psalm/PHPStan tag aliases** — `@psalm-param`, `@psalm-return`, `@psalm-var`, `@phpstan-param`, `@phpstan-return`, `@phpstan-var` are treated identically to their standard counterparts, so codebases using Psalm or PHPStan-style annotations get full hover, completion, and inlay-hint support
...
- **Workspace symbols** — fuzzy-search symbols across the entire project; **kind-filter prefix syntax** — prefix the query with `#class:`, `#fn:`, `#method:`, `#interface:`, `#enum:`, `#const:`, or `#prop:` to restrict results to a single symbol kind (e.g. `#class:User` returns only class symbols matching "User"); **`workspaceSymbol/resolve`** fills in source ranges lazily for clients that request them
