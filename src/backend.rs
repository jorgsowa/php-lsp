...
mod tests {
    ...
    fn position_to_offset_second_line() {
        let src = "<?php\nfoo();";
        // Line 1, character 0 → byte 6 (after '\n')
        assert_eq!(
            position_to_offset(
                src,
                Position {
        ...
        // Line 1, character 3 → byte 9 ("foo" ends here)
        assert_eq!(
            position_to_offset(
                src,
                Position {
    ...
    fn cursor_on_method_decl_name_returns_true() {
        ...
        // Cursor just past the name should not match
        assert!(!cursor_is_on_method_decl(
            src,
            stmts,
            Position {
    ...
    fn cursor_on_free_function_decl_returns_false() {
        ...
        let doc = ParsedDoc::parse(src.to_string());
        let stmts = &doc.program().stmts;
        // "add" starts at col 9 on line 1 — this is a free function, not a method
        assert!(!cursor_is_on_method_decl(
            src,
            stmts,
            Position {
    ...
    fn cursor_on_method_call_site_returns_false() {
        // The call site "$c->add()" has no method declaration — should return false
        let src = "<?php\nclass C { public function add() {} }\n$c = new C();\n$c->add();";
        let doc = ParsedDoc::parse(src.to_string());
        let stmts = &doc.program().stmts;
        // "$c->add()" — "add" starts at col 4 on line 3
        assert!(!cursor_is_on_method_decl(
            src,
            stmts,
            Position {
...
mod integration {
    ...
    async fn references_on_method_decl_returns_method_refs_not_function_refs() {
        ...
        // Must include the method declaration (line 3) and the method call (line 6).
        assert!(
            lines.contains(&3),
            "method declaration on line 3 must be included, got lines: {:?}",
            lines
        );
        assert!(
            lines.contains(&6),
            "method call on line 6 must be included, got lines: {:?}",
            lines
        );
        // Must NOT include the free-function declaration (line 1) or call (line 5).
        assert!(
            !lines.contains(&1),
            "free-function declaration on line 1 must be excluded, got lines: {:?}",
            lines
        );
        assert!(
            !lines.contains(&5),
            "free-function call on line 5 must be excluded, got lines: {:?}",
            lines
        );
    }
